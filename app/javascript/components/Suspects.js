...
// reference Crimes.js to create a SuspectEditor component
      ...
      setOptions(
        response.criminals.map((criminal) => {
          const criminalAlreadyExists = !!find(currentSuspects, {
            data: {
              attributes: { criminal: { data: { id: criminal.data.id } } },
            },
          });
          return {
...
// reference Crimes.js to create a Suspects component
function Suspects({ suspects, investigationId }) {
  ...
  function drop(criminalId) {
    put(`/v1/drop_suspect/${criminalId}`).then((result) => {
      const index = currentSuspects.findIndex(
        (suspect) => suspect.data.id === criminalId
      );
            ...
            return (
              <li key={index}>
                ...
                <ul>
                  ...
                  <li key={index}>
                    {" "}
                    &bull; Dropped: {dropped_on}&nbsp;&nbsp;
                    {suspectData.dropped_on === null && (
                      <button onClick={() => drop(suspect.data.id)}>
                        Drop
                      </button>
                    )}
                  </li>
